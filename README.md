...
Fully offline - SQLite in place of Postgres, fakeredis in place of Redis, and the sample API wired in-process via httpx's ASGI transport in place of a network hop. No Docker daemon, no network access, no secrets required.
...
This project is maintained as a focused gateway solution: no multi-tenant org/RBAC layer above "agent," no Alembic migration chain (the app creates its schema at startup via create_all, which is appropriate for a project of this scope), and the rate/budget reserve-then-compensate pattern is correct for one agent's sequential tool calls but isn't a Lua-script-atomic under extreme concurrency - the extension point (RateLimiter._redis) is called out in the code for exactly that upgrade if you needed it.
...
Lokesh Addanki is a Full Stack Developer with over 5 years of experience in finance, insurance, and automotive domains. He specializes in delivering cloud-native microservices and high-reliability systems using Java, Python, and Node.js.

Email: codeknight.adloky@gmail.com